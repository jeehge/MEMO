...
final class MemoCreateViewController: BaseViewController {
	...
	private func setMemoData() {
		...
		guard let infoMemo = self.updateMemo else { return }
	...
	private func bind() {
		...
		imageSelectButton.rx.tap.bind { [weak self] in
			guard let self = self else { return }
//			let actionSheetVC: ImageActionSheetController = ImageActionSheetController.viewController(from: .custom)
...
//			self.present(actionSheetVC, animated: true)
	...
	private func registerMemoInfo() {
		if isEmpty() == true {
			...
			var strTag = ""
			let arrTag = self.hashTagTextView.text.components(separatedBy: [" ", "\n", "\t", "#"])
			for tag in arrTag {
				if !tag.isEmpty {
					strTag += "#" + tag + " "
				}
			}
			strTag = strTag.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
			
			if self.updateMemo == nil { // 메모 등록
				if let mPath = self.imagePath {
...
//		self.imageDeleteButton.isHidden = false
//		
//		self.imagePath = drawImage.filePathCreate()
//		self.memoImageView.image = drawImage
//	}
//}
